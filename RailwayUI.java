...
public class RailwayUI extends JFrame implements ActionListener {
  ...
  public RailwayUI() {
    ...
    /*
     * The following delcarations initialise the variables for the positions of the
     * blocks
     */
    ...
    /*
     * The next lines of code perform the same processes as the ones before such as
  ...
  public void actionPerformed(ActionEvent ev) {
    selectedblock = block.getSelectedItem(); // The values neeeded to book seats and check for availability taken from
                                             // text boxes
    ...
    if (ev.getSource() == submitbut) {
      ...
      /*
       * The following set of if statements checks for seat number and converts it to
       * an X, Y position which can be used for booking
       */
      ...
      if (selectedtime == "13:00") // If they are trying to book a ticket a 1 o'clock
      {
        ...
        {
          int iftaken = leftside1[convertedseat1][convertedseat2].isTaken(); // Uses the istaken() method to see if it
                                                                             // is taken
          if (iftaken == 1) {
            JOptionPane.showMessageDialog(null, "This seat is taken and cannot be booked"); // Seat is taken and outputs
                                                                                            // an error message
          } else if (iftaken == 0) {
            leftside1[convertedseat1][convertedseat2].setSeat(); // Sets the seat to booked
            repaint(); // Re-displays it
            tickets[totaltickets] = new Ticket(tickettype, selectseat, selectedblock, selectedtime); // Creates a new
                                                                                                     // ticket objects
                                                                                                     // in the tickets
                                                                                                     // array
        ...
        {
          int iftaken = midside1[convertedseat1][convertedseat2].isTaken(); // Uses the istaken() method to see if it is
                                                                            // taken
          if (iftaken == 1) {
            JOptionPane.showMessageDialog(null, "This seat is taken and cannot be booked"); // Seat is taken and outputs
                                                                                            // an error message
        ...
        {
          int iftaken = rightside1[convertedseat1][convertedseat2].isTaken(); // Uses the istaken() method to see if it
                                                                              // is taken
          if (iftaken == 1) {
            JOptionPane.showMessageDialog(null, "This seat is taken and cannot be booked"); // Seat is taken and outputs
                                                                                            // an error message
    ...
    if (ev.getSource() == showbut) {
      /*
       * If the user has clicked the show button it sets a variable and then repaints
       * the desired time
       */
    ...
    if (ev.getSource() == paybut) // Pay button to generate tickets
    {
      ...
      JOptionPane.showMessageDialog(null, "These tickets have been printed out to the file 'tickets.txt'"); // Success
                                                                                                            // message
...
class Seat {
  ...
  public void display(Graphics graf) {
    ...
    case 0:
      ...
      graf.drawString(Integer.toString(seatnumber), x + 20, y + boxheight * 3 / 4); // Painting the seat with the seat
